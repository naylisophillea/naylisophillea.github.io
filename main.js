...
event.preventDefault();// stops the browser from executing its default buuilt in action associated w an event

let data = new FormData(signupForm); //grabs all the names in one go
...
decorationMenu.addEventListener("click",function(event){
    ...
    deco.style.left=Math.random()*220+40+"px";

    deco.style.top=Math.random()*120+40+"px";
